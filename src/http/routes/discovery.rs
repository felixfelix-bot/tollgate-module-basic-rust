...
pub async fn handle_discovery(
    ...
    let metric = if config.metric == "bytes" { "data" } else { "time" };
    let first_mint = config.accepted_mints.first();
    let price = first_mint.map(|m| m.price_per_step.to_string()).unwrap_or_default();
    let unit = first_mint.map(|m| m.price_unit.clone()).unwrap_or_default();
    let url = first_mint.map(|m| m.url.clone()).unwrap_or_default();
    let min_steps = first_mint.map(|m| m.min_purchase_steps.to_string()).unwrap_or_default();
    ...
    let tags = vec![
        ...
        vec!["block".to_string(), "cashu".to_string(), price, unit, url, min_steps],
        vec!["tips".to_string()],
    ];
